...
export const StudioChatCAD: React.FC<StudioChatCADProps> = ({
  ...
  const processUserCommand = async (userText: string): Promise<ProcessCommandResult> => {
    ...
    // Try adding an OH group or highlighting atom 0
    if (parentConformer && parentConformer.atoms && parentConformer.atoms.length > 0) {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden bg-[#070b14] text-slate-100 relative">
      {/* Hidden File Input */}
      <input
        ref={fileInputRef}
        type="file"
        ...
        <div className="p-3.5 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <img
              ...
              className="w-5 h-5 rounded-full object-cover border border-indigo-400/40 shadow-sm shadow-indigo-500/30"
            />
        ...
        <header className="h-14 border-b border-slate-800 bg-[#090e1a]/90 backdrop-blur-md px-4 flex items-center justify-between z-20">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <img
                src="/Pharm_AI_Synapse_Logo.png"
                alt="Pharm AI Synapse"
                className="w-8 h-8 rounded-full object-cover border border-cyan-400/60 shadow-md shadow-cyan-500/30"
              />
              <div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl text-xs font-bold bg-indigo-600/40 hover:bg-indigo-600 border border-indigo-500/60 text-indigo-100 hover:text-white transition-all shadow-md flex items-center gap-1.5 cursor-pointer"
              title="One-Click Export PDF Research Brief with A.P. Anirudh attribution"
            >
              <span>📄</span> Export Research Brief
            </button>
          </div>
        </header>
                  ...
                  <div className="flex items-center gap-1.5 text-[10px] text-slate-400 px-1">
                    {msg.sender !== 'user' && (
                      <img
                        src="/Pharm_AI_Synapse_Logo.png"
                        alt="Pharm AI Synapse"
                        className="w-3.5 h-3.5 rounded-full object-cover border border-cyan-400/60 shadow-sm"
                      />
                    )}
